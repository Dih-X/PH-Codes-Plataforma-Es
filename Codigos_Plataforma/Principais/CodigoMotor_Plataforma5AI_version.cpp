...
enum EtapaTrocaBateria {
    TROCA_INICIO,
    TROCA_ABRIR_GARRA_1,
    TROCA_ESTENDER_1,
    TROCA_FECHAR_GARRA_1,
    TROCA_RETRAIR_1,
    TROCA_DESCER_Z,
    TROCA_ESTENDER_2,
    TROCA_ABRIR_GARRA_2,
    TROCA_FECHAR_GARRA_2,
    TROCA_RETRAIR_2,
    TROCA_SUBIR_Z,
    TROCA_ESTENDER_3,
    TROCA_ABRIR_GARRA_3,
...
EtapaTrocaBateria etapaTroca = TROCA_INICIO;
unsigned long inicioEtapaTroca = 0;
unsigned long pausaEtapaTroca = 0;
bool etapaTrocaIniciada = false;
//Velocidades////////////////////////////////////////////////////
...
void ZERO_X(){ 
    if (digitalRead(Xstart) == LOW && digitalRead(X2start) == LOW){
...
void prepararEtapaTrocaBateria(){
    inicioEtapaTroca = millis();
    etapaTrocaIniciada = true;

    switch (etapaTroca){
        case TROCA_INICIO:
            pausaEtapaTroca = 1000;
            break;

        case TROCA_ABRIR_GARRA_1:
            abrirGarraBateria();
            pausaEtapaTroca = 5000;
            break;

        case TROCA_ESTENDER_1:
            motorZYgarra.moveTo(passosZYgarra);
            pausaEtapaTroca = 6000;
            break;

        case TROCA_FECHAR_GARRA_1:
            fecharGarraBateria();
            pausaEtapaTroca = 5000;
            break;

        case TROCA_RETRAIR_1:
            motorZYgarra.moveTo(0);
            pausaEtapaTroca = 6000;
            break;
        ...
        case TROCA_DESCER_Z:
            motorZ.moveTo(0);
            motor2Z.moveTo(0);
            pausaEtapaTroca = 10000;
            break;

        case TROCA_ESTENDER_2:
            motorZYgarra.moveTo(passosZYgarra);
            pausaEtapaTroca = 6000;
            break;

        case TROCA_ABRIR_GARRA_2:
            abrirGarraBateria();
            pausaEtapaTroca = 5000;
            break;

        case TROCA_FECHAR_GARRA_2:
            fecharGarraBateria();
            pausaEtapaTroca = 5000;
            break;

        case TROCA_RETRAIR_2:
            motorZYgarra.moveTo(0);
            pausaEtapaTroca = 6000;
            break;
        ...
        case TROCA_ESTENDER_3:
            motorZYgarra.moveTo(passosZYgarra);
            pausaEtapaTroca = 6000;
            break;

        case TROCA_ABRIR_GARRA_3:
            abrirGarraBateria();
            pausaEtapaTroca = 5000;
            break;

        case TROCA_RETRAIR_3:
            motorZYgarra.moveTo(0);
            pausaEtapaTroca = 6000;
            break;
...
bool movimentoEtapaTrocaConcluido(){
    switch (etapaTroca){
        case TROCA_INICIO:
        case TROCA_FINALIZADA:
            return true;
        ...
        case TROCA_FECHAR_GARRA_2:
        case TROCA_ABRIR_GARRA_3:
            return motorZgarra.distanceToGo() == 0;
        ...
        case TROCA_ESTENDER_3:
        case TROCA_RETRAIR_3:
            return motorZYgarra.distanceToGo() == 0;
...
void executarTrocaBateria(){
    if (!etapaTrocaIniciada){
        prepararEtapaTrocaBateria();
        return;
    }

    if (!movimentoEtapaTrocaConcluido()){
        return;
    }

    if (millis() - inicioEtapaTroca < pausaEtapaTroca){
        return;
    }

    etapaTroca = static_cast<EtapaTrocaBateria>(etapaTroca + 1);
    etapaTrocaIniciada = false;
}

float calcularDistancia(float diametro, int passos){ //era void essa funcao
...
void setup(){
    ...
    pinMode(sensorPouso, INPUT_PULLUP);
...
void loop(){
    ...
    switch (estadoatual){
        ...
        case HUNT:  //Move a barra Y com as Garras X simultaneamente em direcao a barra de empurrao Y para capturar o drone e alinha-lo
            ...
            if (digitalRead(Yend) == LOW){      //Talvez seja necessario fazer com que o if acione um tempo depois que o motor partir para nao causar possiveis travamentos 
            ...
            if (digitalRead(Xend) == LOW && digitalRead(X2end) == LOW){  //a Garra X so para quando ambas as pincas tocam no drone dos dois lados, cada uma de cada lado
                pararX();
            }else{
                moverX();
            }
